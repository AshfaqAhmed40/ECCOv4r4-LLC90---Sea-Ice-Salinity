...
THETA = squeeze(THETA(:,:,tile,depth)); %taking only the Arctic slice upto 2 km
THETA = flip(THETA);
...
% Fram Strait (80,53)
FramStrait_SST = squeeze(THETA(80,53,:));

% Barents Sea (53,27)
BarentsSea_SST = squeeze(THETA(53,27,:));

% Beaufort Gyre (3,66)
BeaufortGyre_SST = squeeze(THETA(3,66,:));
...
for i = 1:n
   nexttile
   contourf(rot90(rot90(THETA(:,:,i))),150,'edgecolor','none'); shading interp; 
   colormap(gca, clri); caxis([-3 2.5]);
   ...
   % plot the grid
        ...
        for idx = 1:size(cellsToHighlight, 1)
            newRow = size(THETA,1) - cellsToHighlight(idx, 1) + 1;
            newCol = size(THETA,2) - cellsToHighlight(idx, 2) + 1;
            xCoords = [newCol-1.5, newCol+1.5, newCol+1.5, newCol-1.5];
            yCoords = [newRow-1.5, newRow-1.5, newRow+1.5, newRow+1.5];
            patch(xCoords, yCoords, 'm'); 
        end
        hold off;
