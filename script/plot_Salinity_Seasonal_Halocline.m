...
switch seasonInput
    case 'w'
        sourceDir = strcat(HOME, 'input/winter/');
        fileName = 'winter_halocline.mat';
    case 'sp'
        sourceDir = strcat(HOME, 'input/spring/');
        fileName = 'spring_halocline.mat';
    case 'sm'
        sourceDir = strcat(HOME, 'input/summer/');
        fileName = 'summer_halocline.mat';
    case 'f'
        sourceDir = strcat(HOME, 'input/fall/');
        fileName = 'fall_halocline.mat';
    otherwise
...
for i = 1:numMonths 
    ...
    plotColor = cmap1(colorIndex, :); ylim([-300 -5])
    plot(ax1, FramStrait(1:L,i), Z(1:L), '-', 'linewidth', 1, 'Color', plotColor); hold on;
end
PLOT(ax1, 'Fram Strait', [29 36], 'Salinity (pss)', 'Depth (m)', numMonths, cmap1);
...
for i = 1:numMonths 
    ...
    plotColor = cmap2(colorIndex, :); ylim([-300 -5])
    plot(ax2, BarentsSea(1:L,i), Z(1:L), '-', 'linewidth', 1, 'Color', plotColor); hold on;
end
PLOT(ax2, 'Barents Sea', [29 36], 'Salinity (pss)', 'Depth (m)', numMonths, cmap2);
...
for i = 1:numMonths 
    ...
    plotColor = cmap3(colorIndex, :); ylim([-300 -5])
    plot(ax3, BeaufortGyre(1:L,i), Z(1:L), '-', 'linewidth', 1, 'Color', plotColor); hold on;
end
PLOT(ax3, 'Beaufort Gyre', [29 36], 'Salinity (pss)', 'Depth (m)', numMonths, cmap3);
