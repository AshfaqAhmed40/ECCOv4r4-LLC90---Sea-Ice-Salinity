...
%% Data
% tic % 178 seconds
% HOME = '/Users/aahmed78/Desktop/PhD/Arctic Ocean Temperature/Ocean Model/ECCOv4r4 LLC0090/';
% cd(strcat(HOME,'input/data/'));
% sourceDir = (strcat(HOME,'input/data/'));
% files = dir(fullfile(sourceDir, '*.nc'));
% THETA_3D = zeros(90, 90, length(files)); % preparing a 3D variable
% 
% 
...
%     THETA = squeeze(THETA(:,:,tile,depth)); % considering Arctic slice
%     THETA = flip(THETA);
%     THETA_3D(:,:,ii) = THETA;
% end
% toc
...
THETAr = reshape(THETA_3D,length(THETA_3D(:,1,1))*length(THETA_3D(1,:,1)),length(THETA_3D(1,1,:)))';
latr = reshape(LAT,1,length(LAT(:,1))*length(LAT(1,:)));
lonr = reshape(LON,1,length(LON(:,1))*length(LON(1,:)));
THETAr = [latr;lonr;THETAr]; 

%% 3. Remove nan data (actually, 100% NaN columns)
THETAr(:,sum(isnan(THETAr(3:end,:)),1)==length(THETAr(3:end,1)))=[];
...
THETAr = fillmissing(THETAr,'linear',1,'EndValues','near');
SST = THETAr;
%SST = rmmissing(SST,2);
...
% Plot the EOF1
nexttile;
[~, hContour]=contourf(rot90(rot90(finaleeof1s).*U1(7,1)),200,'edgecolor','none');
set(gca,'Color',[0.65 0.65 0.65]); shading interp;
colormap(gca,clri); caxis([-1e-3 1.e-3]);
...
% Plot the EOF2
nexttile
[~, hContour]=contourf(rot90(rot90(finaleeof2s).*U2(7,1)),200,'edgecolor','none');
set(gca,'Color',[0.65 0.65 0.65]); shading interp;
colormap(gca,clri); caxis([-1e-3 1.e-3]);
set(gca, 'XTickLabel',[]); set(gca, 'YTickLabel',[]);
hold on; box on; 
...
% Plot the EOF3
nexttile
[~, hContour]=contourf(rot90(rot90(finaleeof3s).*U3(6,1)),200,'edgecolor','none');
set(gca,'Color',[0.65 0.65 0.65]); shading interp;
colormap(gca,clri); caxis([-1e-3 1.e-3]);
set(gca, 'XTickLabel',[]); set(gca, 'YTickLabel',[]);
hold on; box on; 
...
%         U = U1;
%         eofFolderName = 'EOF1-Temperature';
%     case 2
%         finaleeof = finaleeof2s;
%         U = U2;
%         eofFolderName = 'EOF2-Temperature';
%     case 3
%         finaleeof = finaleeof3s;
%         U = U3;
%         eofFolderName = 'EOF3-Temperature';
%     otherwise
...
loglog(f,1000.*pxx1,'k-','linewidth',2); hold on
