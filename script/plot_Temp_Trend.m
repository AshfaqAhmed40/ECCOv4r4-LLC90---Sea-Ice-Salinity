...
ylabel('Temperature (°C)'); ylim([-2.5 1.5])
title('Annual trend (95% Confidence)');
subtitle(sprintf('Mean temperature upto %.1f m deep',Z(L))); 
box on; set(gca, 'GridLineStyle', '-', 'LineWidth', 3); hold off
set(gca,'XTickLabel',[]); 
legend('Fram Strait','location','northwest','fontsize',20);
...
box on; set(gca, 'GridLineStyle', '-', 'LineWidth', 3); hold off
set(gca,'XTickLabel',[]);
legend('Barents Sea','location','northwest','fontsize',20);
...
legend('Beaufort Sea','location','northwest','fontsize',20);
