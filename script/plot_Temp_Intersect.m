...
intrsctd_SSS{1} = fliplr(squeeze(THETA(:,90-xIdx,:))'); %against xIdx
intrsctd_SSS{2} = fliplr(squeeze(THETA(90-yIdx,:,:))'); %against yIdx

% Plotting
nexttile
contourf(rot90(rot90(THETA(:,:,1))),100,'edgecolor','none'); shading interp; 
colormap(gca, clri); set(gca,'color',[0.55 0.55 0.55]);
line([xIdx, xIdx], [1, 90], 'Color', 'c', 'LineWidth', 4); hold on;
...
title('Temperature with depth (cyan intersect)'); 
...
box on; set(gca, 'GridLineStyle', '-','LineWidth', 3);
hold on; set(gca, 'Layer', 'top');
title('Temperature with depth (red intersect)'); 
set(gca,'XTickLabel',[]); set(gca,'YTickLabel'); 
ylabel('Depth (m)'); set(gca,'FontSize', 15)
...
c.Layout.Tile = 'east'; caxis([-5 2]); clc;